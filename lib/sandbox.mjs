...
import { root, stateRoot, agentDir, sharedSkills, canonical } from './paths.mjs';
...
export async function executeSandboxed({ command, cwd, readOnly = false, domains = [], networkAccess = false, signal, timeout = 120, onData = () => {} }) {
  ...
  const config = {
    ...
    filesystem: {
      ...
      denyWrite: [agentDir, '/tmp/claude', '/private/tmp/claude', join(homedir(), '.npm/_logs'), join(homedir(), '.claude/debug'), join(cwd, '.git'), join(cwd, '.pi'), join(cwd, '.agents')].map(canonical),
    },
