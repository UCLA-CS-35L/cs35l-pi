...
export function projectPaths(workspace) {
  ...
  if ([agentDir].some(base => within(canonical(workspace), canonical(base)) || within(canonical(base), canonical(workspace)))) throw new Error('Choose a workspace separate from private Pi state.');
